...
@Component({
  selector: 'app-reporte-populares',
  templateUrl: './reporte-populares.component.html',
  styleUrls: ['./reporte-populares.component.css']
})
export class ReportePopularesComponent implements OnInit {
  ...
  mostrarReporte(){
   ...
   this.reportComentario=this.reporteAdminService.verReporteRevistaPopular(this.reporteForm.get("revista")?.value,this.fechaI,this.fechaF);
    this.mostrar=true;
  }

  descargarReporte(){
    console.log(this.fechaF + this.fechaI + this.reporteForm.get("revista")?.value);
    const revi =this.reporteAdminService.descargarReporteRevistaPopular(this.reporteForm.get("revista")?.value,this.fechaI,this.fechaF);

...
export class ReporteComentarioComponent implements OnInit {
  ...
  mostrarReporte(){
   ...
   this.reportComentario=this.reporteService.verReporte(this.reporteForm.get("revista")?.value,this.fechaI,this.fechaF);
    this.mostrar=true;
  }

  descargarReporte(){
    console.log(this.fechaF + this.fechaI + this.reporteForm.get("revista")?.value);
    const revi =this.reporteService.descargarReporte(this.reporteForm.get("revista")?.value,this.fechaI,this.fechaF);
